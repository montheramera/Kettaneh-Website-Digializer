...
export async function generateMetadata({ params }: Props) {
  try {
    const res = await fetch(`${API_URL}/api/cookies-policy?populate=seo.fav_icon`, {
      cache: "no-store",
    })
    ...
    const title = seo.meta_title || "cookies-policy";
    const description = seo.meta_description || "cookies-policy";
    ...
    return {
      ...
      description,
      alternates: {
        canonical: `${process.env.NEXT_PUBLIC_MAIN_SITE}/en/cookies-policy`,
      },
    };
  } catch (error) {
...
const fetchCookiesData = async () => {
  const res = await fetch(`${API_URL}/api/cookies-policy?populate=*`, {
    method: "GET",
    headers: {
          ...
          <section className="max-w-[1440px] m-auto">
            <div>
              ...
              <Suspense fallback={"loading"}>
                <DynamicParagraph content={cookiesData?.text} classes="text-[18px] lg:text-[20.1px] font-[400] leading-[28px] text-paragraph lg:my-4 lg:max-w-[1216px]" />
              </Suspense>
