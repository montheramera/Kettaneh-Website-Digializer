          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            <div className="lg:col-span-2">
              ...
              <p className="text-sm lg:text-base text-gray-500 mt-4 mb-8">
                {new Date(event.data.attributes.Event.date).toLocaleDateString(
                  "en-US",
                  {
                    year: "numeric",
                    month: "long",
              ...
              <BlocksRendererComponent
                content={event.data.attributes.Event?.summary}
              />
            </div>

            {/* Featured Events (Read Next) */}
            <div className="lg:col-span-1">
              <h2 className="text-white bg-primary inline-block px-6 py-1 text-2xl font-bold text-gray-800 mb-4">
                Read Next
              </h2>
              <div className="space-y-8">
                {FeaturedEvents?.map((event: any, index: number) => (
                  <Link
                    href={`/en/news-and-events/${event.Event.title?.replaceAll(
                      " ",
                      "-"
                    )}`}
                    key={index}
                    aria-label={`Go to the event: ${event.Event.title}`}
                    className="mb-4"
                  >
